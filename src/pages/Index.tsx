...
const Index = () => {
  ...
  // Courier detection functions
  const detectCourierAdvanced = (trackingNumber) => {
    // Clean the input: remove spaces, hyphens, convert to uppercase
    const clean = trackingNumber.replace(/[\s\-]/g, '').toUpperCase();
    
    // UPS Detection - These patterns are unique to UPS
    ...
    // UPS 9-digit format - validate using length and no letter pattern
    if (clean.match(/^\d{9}$/) && !clean.match(/^[0-2]/)) {
        return { courier: 'UPS', format: '9-digit', confidence: 90 };
    }
    
    // FedEx SmartPost - Very specific patterns
    ...
    if (clean.match(/^\d+$/)) {
        ...
        // Purolator fallback for 12-digit not starting with 3
        if (length === 12) {
  ...
  const validateUPSChecksum = (trackingNumber) => {
    ...
    const sequence = clean.substring(2, 17); // Remove "1Z" and check digit
    ...
    for (let i = 0; i < sequence.length; i++) {
        ...
        if (/\d/.test(char)) {
            ...
            // Convert letter to number (A=2, B=3, ..., Z=0)
  ...
  const validateAndDetectCourier = (trackingNumber) => {
    ...
    // Add UPS checksum validation for higher confidence
